...
fn bench_merkle_tree_creation(c: &mut Criterion) {
    ...
    for length in lengths {
        ...
        let strings: Vec<String> = (1..=length)
        ...
        c.bench_function(format!("merkle_tree_creation_{}", length).as_str(), |b| b.iter(|| {
            black_box(MerkleForest::new(data_blocks.clone()));
        }));
    }
}

fn bench_merkle_tree_add_entry(c: &mut Criterion) {
    let lengths = vec![2u32.pow(5) - 1, 2u32.pow(10) - 1, 2u32.pow(15) - 1, 2u32.pow(20) - 1];
    for length in lengths {
        // Create a vector of different data blocks
        let strings: Vec<String> = (1..=length)
            .map(|i| format!("block{}", i))
        ...
        c.bench_function(format!("merkle_tree_add_entry_{}", length).as_str(), |b| b.iter(|| {
